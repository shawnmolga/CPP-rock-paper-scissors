...
bool RCPgameManager::checkPositioningFormat(const string &line,
	...
	if (isJoker) {
		...
		if (line[6] != ROCK || line[6] != PAPER || line[6] != SCISSOR
				|| line[6] != BOMB) {
...
bool RCPgameManager::checkBadFormat(string &positionFile1,
		string &positionFile2) {
...
bool RCPgameManager::isLegalMove(int from_x, int from_y, int to_x, int to_y,
	...
	if ((from_x < 1 || from_x > 1 - ROWS) || (to_x < 1 || to_x > 1 - ROWS)
			|| (from_y < 1 || from_y > 1 - COLS)
			|| (to_y < 1 || to_y > 1 - COLS)) {
		cout << "Error: illegal location on board" << endl;
		return false;
	}
	...
	if (game.board[from_x][from_y].getPiece() != 0) {
		if (isupper(game.board[from_x][from_y].getPiece()) == true
				&& isPlayer1 == true) {
		...
		} else if (isupper(game.board[from_x][from_y].getPiece()) == false
				&& isPlayer1 == false) {
...
bool RCPgameManager::makeMove(string s, bool isPlayer1) {
	...
	if (game.board[x_joker][y_joker].getIsJoker() == false) { //if the original peice is not a joker
