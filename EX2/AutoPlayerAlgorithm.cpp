...
void AutoPlayerAlgorithm::positionUnmovingPieces(int player, std::vector<unique_ptr<PiecePosition>> &vectorToFill) {
	...
	if (protectingBombsNum < 1 && BOMBS_NUM > 0) { //prefer to protect flags
		...
		const std::vector<unique_ptr<FightInfo>> &fights)
{
	...
	for (int i = 0; i < fights.size(); ++i) {
		RPSpoint p = fights[i]->getPosition();
...
unique_ptr<JokerChange> AutoPlayerAlgorithm::getJokerChange() {
	...
	for (int i = 0; i < ROWS; ++i) {
		for (int j = 0; j < COLS; ++j) {
			...
			bool isMyPiece = (gameBoard.board[i][j].isPlayerOnePiece() && (myPlayerNum == 1)) || (!gameBoard.board[i][j].isPlayerOnePiece() && (myPlayerNum != 1));
...
void AutoPlayerAlgorithm::getBestMove(int &fromRow, int &fromCol, int &toRow, int &toCol) {
	...
	else { //there is going to be a fight
...
double AutoPlayerAlgorithm::tryMovePiece(Move& move
	...
	if (gameBoard.board[to_x][to_y].getPiece() == 0) {
		...
		bool lostPieceInFight = tryToFight(isPlayerOneTurn, to_x, to_y,
...
bool AutoPlayerAlgorithm::tryToFight(int to_x, int to_y, char myPiece, bool isJoker) {
	...
	if (opponentPiece != '#') { //we know the opponent piece so we can simulate fight normally
		return fight();
	}
	else {
		int prob = getRandomNumInRange(1, 100);
		if (prob < 50) {//guess i will lose in fight
...
bool RPSGame::fight(int row, int col, char myPiece, char opponentPiece, bool isMyPieceJoker) {

	//Case 1: 2 players in the same type - tie
	if (myPiece == opponentPiece) {
		Cell::cleanCell(board[row][col]);
		return false;
	}

	//Case 2: there is flag and current player has another piece
	else if (opponentPiece == FLAG) {
			Cell::updateCell(board[row][col], myPiece, isMyPieceJoker);
			return true;
	}
...
void notifyFightResult(const FightInfo& fightInfo) {
	...
	if (movingPieceNumOnBoard == 0) { //we can mark all pieces as not moving pieces !!!
		updateMovingPiece();
	}
...
char AutoPlayerAlgorithm::shouldChangeJoker(int &score,int joker_x, int joker_y, bool amIPlayerOne) {
...
void AutoPlayerAlgorithm::updateMovingPiece() {
	for (int i = 0; i < ROWS; ++i) {
		for (int j = 0; j < COLS; ++j) {
			...
			if (gameBoard.board[i][j].getPiece() == '#' || myPlayerNum != piecePlayerNum) {//this is opponent's piece
				if (gameBoard.board[i][j].isMovingPieceKnown && gameBoard.board[i][j].isMovingPiece) {
					continue;
				}
...
void AutoPlayerAlgorithm::updateBoard(const Board &b) {
	for (int i = 0; i < ROWS; ++i) {
		for (int j = 0; j < COLS; ++j) {
			...
			if (b.getPlayer(p) != 0) { //not empty - must be opponent piece
